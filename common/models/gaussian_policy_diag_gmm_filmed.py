...
class FilmedGMMPolicyDiag(GaussianPolicyDiag):
    """
    ...
    def __init__(self, obs_dim, action_dim, init, hidden_sizes=(64, 64), std_hidden_sizes=(64, 64), n_components=1,
                 ...
                 fix_mean_bias: bool = False, **kwargs):
    ...
    def _get_mean(self, action_dim, prev_size=None, init=None, gain=0.01, scale=1e-4):
        ...
        mean.bias.data = self.distribute_components(self.n_components)
    ...
    def forward(self, x: ch.Tensor, train: bool = True):
        ...
        std = (self.diag_activation(flat_chol + self._pre_activation_shift) + self.minimal_std)

        std = std.diag_embed().expand(x.shape[:-1] + (self.n_components, -1, -1))
